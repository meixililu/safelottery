...
public class CZ14LotteryParser {
	//300
	//解析开奖结果
	public static AutoWrapView getLotteryResultView(Context context,String lid, String[] results){
		...
		if(results != null){
			for(int i=0; i<results.length; i++){
				TextView btn = ViewUtils.Bla_scheme_For_JC(context,(results[i]),"red","");
	...
	public static void getUnkonwResultView(Context context,AutoWrapView linearLayout,String result,String playMethod){
		...
		if(map != null){
			...
			for(int j=0; j<len; j++){
				String[] t = map.get(j+"");
				if(j >= 1){
					TextView btn = ViewUtils.Bla_scheme_For_JC(context,",","split","black");
					linearLayout.addView(btn);
				}
				for(int i=0; i<t.length; i++){
					TextView btn = ViewUtils.Bla_scheme_For_JC(context,(t[i]),"","black");
					linearLayout.addView(btn);
				}
			}
		...
		if(map != null){
			...
			for(int j=0; j<len; j++){
				...
				for(int i=0; i<t.length; i++){
					...
					if(isRight > 0){
						TextView btn = ViewUtils.Bla_scheme_For_JC(context,(t[i]),"","red");
						linearLayout.addView(btn);
					}else{
						TextView btn = ViewUtils.Bla_scheme_For_JC(context,(t[i]),"","black");
						linearLayout.addView(btn);
