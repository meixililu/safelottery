...
public class JZBetActivity extends JCBaseActivity {

	public static String[] items = {"胜平负", "让球胜平负", "全场比分", "总进球数", "半全场","混合过关"};
				...
				public void onSuccess(int statusCode, Map mMap) {
					if(mMap != null){
						...
						try {
							...
							LogUtil.DefalutLog("Spend Time:" + (end-begin));
						} catch (Exception e) {
	...
	private void showItems(final JZMatchBean bean, LinearLayout parent_layout) {
		if (playMethod.equals(JZActivity.WF_RQSPF)) {
			...
			mJCItemClass.showItems(imageLoader, bean, parent_layout);
			...
			if(!(mJCItemClass instanceof JZQCBFCLass))
				mJCItemClass = new JZQCBFCLass(this, inflater, sectionAdapter);
			mJCItemClass.showItems(imageLoader, bean, parent_layout);
		} else if (playMethod.equals(JZActivity.WF_JQS)) {
			if(!(mJCItemClass instanceof JZJQSCLass))
				mJCItemClass=new JZJQSCLass(this, inflater, sectionAdapter);
			mJCItemClass.showItems(imageLoader, bean, parent_layout);
		}else if (playMethod.equals(JZActivity.WF_BQC)) {
			if(!(mJCItemClass instanceof JZBQCCLass))
				mJCItemClass=new JZBQCCLass(this, inflater, sectionAdapter);
			mJCItemClass.showItems(imageLoader, bean, parent_layout);
		}else if (playMethod.equals(JZActivity.WF_HHGG)) {
			if(!(mJCItemClass instanceof JZHHGGCLass))
				mJCItemClass=new JZHHGGCLass(this, inflater, sectionAdapter);
			mJCItemClass.showItems(imageLoader, bean, parent_layout);
		}
	}
	...
	public void top_menu(View v) {
		super.top_menu(v);
		final Context mContext = JZBetActivity.this;
		if(popDialog == null){
	...
	private int selectPlayMethodId(String playMethodName){
		if(playMethodName.equals(JZActivity.WF_SPF)){
			...
			return 4;
		}else if(playMethodName.equals(JZActivity.WF_HHGG)){
			return 5;
		}
		return 0;
	}
	...
	public void onBackPressed() {
		if(selectNumber>0){
			NormalAlertDialog dialog = new NormalAlertDialog(JZBetActivity.this);
			dialog.setTitle("提示");
